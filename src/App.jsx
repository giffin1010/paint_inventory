      ...
      {options.map((option) => {
        ...
        return (
          <button
            ...
            className={`min-h-11 rounded-full border px-4 py-2.5 text-[15px] font-medium leading-none transition-colors ${
              active
...
function PrimLogo() {
  return (
    <div className="flex items-center gap-0 text-[26px] font-black leading-none tracking-tight sm:text-3xl">
...
export default function PaintInventoryApp() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(collection(db, 'paints'), (snapshot) => {
      ...
      syncedItems.sort((a, b) => (b.createdAt ?? 0) - (a.createdAt ?? 0))
  ...
  async function handleSaveItem() {
    ...
    if (editingId !== null) {
      await updateDoc(doc(db, 'paints', editingId), payload)
    } else {
  ...
  function editItem(item) {
    ...
    const knownProduct = item.product ?? ''
  ...
  return (
    <div className="min-h-screen bg-neutral-100 px-2 py-2 sm:p-6">
      <div className="mx-auto w-full max-w-5xl">
        <Card className="overflow-hidden rounded-[28px] border-0 bg-transparent shadow-xl sm:bg-white">
          ...
          <CardContent className="space-y-3 p-2 sm:space-y-4 sm:p-6">
            <div className="grid grid-cols-2 gap-2 sm:gap-3">
              <button
                ...
                className={`rounded-2xl text-left transition-all duration-200 active:scale-[0.99] ${
                  filter === 'interior' ? 'bg-blue-50 shadow-md ring-2 ring-blue-400' : ''
                }`}
              >
              ...
              <button
                ...
                className={`rounded-2xl text-left transition-all duration-200 active:scale-[0.99] ${
                  filter === 'exterior' ? 'bg-green-50 shadow-md ring-2 ring-green-400' : ''
                }`}
              >
                <Card className="rounded-2xl border-0 bg-white/95 shadow-sm">
                  <CardContent className="p-3 sm:p-4">
            ...
            <div className="grid gap-2.5 pb-3 sm:gap-3 sm:pb-0">
              {filteredItems.length === 0 ? (
                ...
                filteredItems.map((item) => (
                  <Card
                    key={item.id}
                    className="cursor-pointer overflow-hidden rounded-3xl bg-white shadow-sm transition-shadow hover:shadow-md"
                  ...
                  >
                    <CardContent className="p-4">
                      <div className="flex flex-col gap-3">
                        ...
                        <div className="grid grid-cols-1 gap-1.5 text-sm text-neutral-600 sm:grid-cols-2 sm:gap-2 lg:grid-cols-4">
